...
if __name__ == '__main__':
	from mgn import mgn
	port 		= mgn.config['PORT']
	ip	 		= mgn.config['IP']
	app_name 	= mgn.config['APP_NAME']
	host_name 	= mgn.config['HOST_NAME']
	...
	if fwtype == "gevent":
		...
		WSGIServer((ip, port), mgn).serve_forever()

	elif fwtype == "cherrypy":
		from cherrypy import wsgiserver
		server = wsgiserver.CherryPyWSGIServer(
			(ip, port), mgn, server_name=host_name)
		server.start()
		...
		make_server(ip, port, mgn).serve_forever()